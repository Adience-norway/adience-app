...
const INPUT  = `${process.env.HOME}/Desktop/arenaer-norge-import.json`;
const OUTPUT = `${process.env.HOME}/Desktop/arenaer-norge-geocoded.json`;
const UA     = "Adience-DataImport/1.0 (lagethune@gmail.com)";

const arenaer = JSON.parse(readFileSync(INPUT, "utf8"));

async function geocode(arena, by) {
  const q   = encodeURIComponent(`${arena}, ${by}, Norge`);
  const url = `https://nominatim.openstreetmap.org/search?format=json&limit=1&q=${q}`;
  const res = await fetch(url, { headers: { "User-Agent": UA, "Accept-Language": "no,en" } });
...
for (let i = 0; i < arenaer.length; i++) {
  ...
  try {
    const geo = await geocode(a.arena, a.by);
    if (geo) {
  ...
  if (i < arenaer.length - 1) await sleep(1100); // 1 req/sek + margin
}
